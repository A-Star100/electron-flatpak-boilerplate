...
- And a vendored tarball; this should have your app's code (main.js, preload.js, etc), any licenses and in-app libraries for them, a README (if you want), any icons and any .desktop or .metainfo.xml files (for Flathub), and the Electron binaries you need (such as electron-v40.0.0-linux-x64.zip from [the official Electron GitHub repo](https://github.com/electron/electron/releases) in the cache/ folder
...
If any more issues occur, as a last resort add the flag `--ozone-platform=x11` to your run.sh's execution portion.
For example:
```yaml
sources:
      - type: script
        ...
        commands:
          ...
          - 'exec zypak-wrapper /app/lib/MyAwesomeApp/MyAwesomeApp --ozone-platform=x11 "$@"' # uses zypak to execute the compiled binary
```
## My builds keeps failing when running `npm install` or due to ENOTCACHED (even if assets are cached)
This means that you're attempting to **get packages from INSIDE the build process**, which isn't a great for [security reasons](https://github.com/flathub/flathub/issues/3392#issuecomment-1207174370).
In order to fix this you will very likely need to practically rewrite your YAML. The YAML in this repository doesn't have these errors because I rewrote it after realizing this.
